...
6. 발신 도메인 인증과 테스트 메일을 마친 뒤 `RESEND_API_KEY`, `ALERT_FROM_EMAIL`, `APP_URL`을 설정하고 `RADAR_EMAIL_ENABLED=1`로 변경. 프리뷰 배포에서는 Vercel Cron이 실행되지 않으므로 알림 예약은 별도 배포·스케줄 검증 후 활성화.
