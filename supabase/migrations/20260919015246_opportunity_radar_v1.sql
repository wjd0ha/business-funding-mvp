...
-- 중복 방지 알림 큐를 추가한다.
-- 새 격리 프로젝트에서는 기존 운영 데이터 없이 필요한 테이블만 생성한다.

create table if not exists public.start_finder_admins (
...
create table if not exists public.alert_leads (
  ...
  business_status text not null check (business_status in ('pre', 'biz', 're')),
  ...
  alert_weekly boolean not null default true,
  alert_d7 boolean not null default true,
  alert_d3 boolean not null default true,
  consent_privacy boolean not null check (consent_privacy),
  consent_service boolean not null check (consent_service),
...
create table if not exists public.alert_notices (
  ...
  also_categories text[] not null default '{}',
  regions text[] not null default array['전국'],
  target_status text[] not null default array['pre', 'biz', 're'],
  min_years numeric,
  max_years numeric,
  industries text[] not null default array['all'],
  min_employees integer,
  notice_type text not null default 'confirmed' check (notice_type in ('confirmed', 'expected')),
  apply_start date,
  apply_end date,
...
create table if not exists public.alert_matches (
  ...
  notice_id text not null references public.alert_notices(id) on delete cascade,
  score integer not null,
  fit text not null check (fit in ('high', 'check')),
  reasons text[] not null default '{}',
  created_at timestamptz not null default now(),
  notified_at timestamptz,
  primary key (lead_id, notice_id)
);

create table if not exists public.alert_saved_notices (
  lead_id uuid not null references public.alert_leads(id) on delete cascade,
  notice_id text not null references public.alert_notices(id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (lead_id, notice_id)
...
create table if not exists public.alert_notifications (
  id bigint generated by default as identity primary key,
  lead_id uuid not null references public.alert_leads(id) on delete cascade,
  kind text not null check (kind in ('instant', 'weekly', 'd7', 'd3')),
...
alter table public.start_finder_admins enable row level security;
...
alter table public.alert_saved_notices enable row level security;
alter table public.alert_events enable row level security;
alter table public.alert_notifications enable row level security;

drop policy if exists "public signs up" on public.alert_leads;
create policy "public signs up" on public.alert_leads for insert to anon
with check (status = 'active' and unsubscribed_at is null and last_notified_at is null
  and consent_privacy and consent_service);
drop policy if exists "public reads active notices" on public.alert_notices;
create policy "public reads active notices" on public.alert_notices for select to anon, authenticated
using (is_active);
drop policy if exists "public logs events" on public.alert_events;
create policy "public logs events" on public.alert_events for insert to anon
with check (char_length(coalesce(notice_id, '')) <= 64);

grant insert on public.alert_leads, public.alert_events to anon;
grant select on public.alert_notices to anon;
grant usage, select on sequence public.alert_events_id_seq to anon;
