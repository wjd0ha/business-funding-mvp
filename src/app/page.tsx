...
function Intro({ onStart, count, demo }: { onStart: () => void; count: number; demo: boolean }) {
  return <>
    <section className="relative overflow-hidden bg-[var(--navy)] text-white">
      ...
      <div className="relative mx-auto grid min-h-[700px] max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[1.05fr_.95fr] lg:px-8">
        <div>
          ...
          <p className="mt-4 text-sm text-[#91a7c8]">{alertsLive ? "가입 없이 결과 먼저 확인 · 알림 신청은 선택" : "가입 없이 결과 먼저 확인 · 프리뷰 관심 등록은 선택"}</p>
        </div>
    ...
    <section id="how" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="max-w-2xl"><p className="eyebrow">HOW IT WORKS</p><h2 className="mt-3 text-3xl font-black sm:text-4xl">검색이 아니라, 내 조건으로 먼저 거릅니다</h2></div><div className="mt-10 grid gap-4 md:grid-cols-3">{[["01", "사업정보 입력", "사업 상태·지역·업종·업력 등 핵심 정보만 간단히 입력합니다."], ["02", "조건별 자동 매칭", "지역·업력·업종·관심분야를 기준으로 관련 공고를 점수화합니다."], ["03", alertsLive ? "신규 기회 알림" : "프리뷰 관심 등록", alertsLive ? "원하면 새 공고와 저장한 공고의 D-7·D-3 알림을 이메일로 받습니다." : "프리뷰에서는 공고를 확인하고 관심을 등록할 수 있습니다. 이메일은 아직 발송하지 않습니다."]].map(([no, title, text]) => <article key={no} className="soft-card p-7"><span className="text-sm font-black text-[var(--blue)]">{no}</span><h3 className="mt-8 text-xl font-black">{title}</h3><p className="mt-3 leading-7 text-[#657187]">{text}</p></article>)}</div></section>
  </>;
}
