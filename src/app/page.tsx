...
function LeadForm({ profile, sessionId, leadId, setLeadId, matchCount }: { profile: RadarProfile; sessionId: string; leadId: string | null; setLeadId: (value: string) => void; matchCount: number }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setState("sending"); const params = new URLSearchParams(window.location.search); const response = await fetch("/api/leads", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...profile, sessionId, name: form.name, email: form.email, phone: form.phone, consentPrivacy: form.privacy, consentService: form.privacy, consentMarketing: form.marketing, utm: Object.fromEntries([...params.entries()].filter(([key]) => key.startsWith("utm_"))) }) }); const data = await response.json(); if (response.ok) { setLeadId(data.leadId); setState("done"); } else { setMessage(data.error || "등록하지 못했습니다."); setState("error"); } }
  if (leadId || state === "done") return <aside id="alert-form" className="h-fit rounded-2xl border border-[#a9dfcd] bg-[#effaf6] p-6 lg:sticky lg:top-24"><span className="grid size-11 place-items-center rounded-full bg-[#20b486] text-xl text-white">✓</span><h2 className="mt-5 text-xl font-black">맞춤 알림 신청 완료</h2><p className="mt-3 text-sm leading-6 text-[#557064]">새 공고와 저장한 공고의 마감 알림을 보내드릴 준비가 됐습니다.</p></aside>;
  return <aside id="alert-form" className="h-fit rounded-2xl bg-[var(--navy)] p-6 text-white lg:sticky lg:top-24"><span className="text-xs font-black tracking-[.14em] text-[#70a9ff]">FREE ALERT</span><h2 className="mt-3 text-2xl font-black">다음 공고를<br />놓치지 마세요.</h2><p className="mt-3 text-sm leading-6 text-[#aebfda]">현재 조건 {matchCount}건을 저장하고 신규 공고·D-7·D-3 알림을 받아보세요.</p><form className="mt-6 grid gap-3" onSubmit={submit}><input aria-label="이름" placeholder="이름 (선택)" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="form-input" /><input aria-label="이메일" type="email" required placeholder="이메일 (필수)" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="form-input" /><input aria-label="휴대폰" inputMode="tel" placeholder="휴대폰 (선택)" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} className="form-input" /><label className="mt-2 flex gap-2 text-xs leading-5 text-[#c3d0e4]"><input required type="checkbox" checked={form.privacy} onChange={(event) => setForm({ ...form, privacy: event.target.checked })} /> <span>맞춤 알림 제공을 위한 개인정보 수집·이용에 동의합니다. (필수)</span></label><label className="flex gap-2 text-xs leading-5 text-[#c3d0e4]"><input type="checkbox" checked={form.marketing} onChange={(event) => setForm({ ...form, marketing: event.target.checked })} /> <span>비즈핏 교육·상담·이벤트 안내를 받습니다. (선택)</span></label>{state === "error" && <p className="rounded-lg bg-[#ffdddd] px-3 py-2 text-xs font-bold text-[#8d2020]">{message}</p>}<button disabled={state === "sending"} className="mt-2 rounded-xl bg-[var(--blue)] px-5 py-4 font-black disabled:opacity-60">{state === "sending" ? "등록 중…" : "무료 맞춤 알림 받기"}</button></form><p className="mt-3 text-center text-[11px] text-[#859ab9]">언제든 수신 해지할 수 있습니다.</p></aside>;
}
