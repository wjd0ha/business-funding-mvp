    ...
    <main className="min-h-screen bg-[var(--mist)] text-[var(--navy)]">
      <header className="sticky top-0 z-30 border-b border-[#dfe8f5] bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="flex items-center gap-3 text-xs font-bold text-[#66738a]"><Link href="/prep-map" className="rounded-lg bg-[#edf4ff] px-3 py-2 text-[var(--blue)]">2027 준비지도</Link><span className="hidden sm:inline">{noticeSource === "demo" ? "예시 모드" : "기업마당 수집 공고"}</span><span className={`size-2 rounded-full ${noticeSource === "demo" ? "bg-[#e5a626]" : "bg-[#20b486] shadow-[0_0_0_5px_rgba(32,180,134,.12)]"}`} /></div>
        </div>
      </header>
  ...
  return <>
    <section className="relative overflow-hidden bg-[var(--navy)] text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-16 text-center lg:px-8 lg:py-20">
        ...
        <p className="mt-4 text-xs text-[#9fb4d3]">표시 건수는 현재 비즈핏 저장소의 공고 수입니다. 기업마당 전체·실시간 접수 건수가 아닙니다. <Link href="/prep-map" className="underline underline-offset-2">3개년 통계의 출처와 집계 기준 보기</Link></p>
      </div>
    </section>
    <section id="how" className="mx-auto max-w-7xl px-5 pt-16 lg:px-8"><p className="eyebrow">HOW BIZFIT WORKS</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">입력한 조건은 이렇게 결과에 반영됩니다</h2><div className="mt-5 grid gap-3 md:grid-cols-3">{[["01","지역·사업 형태","공고에 표시된 지역·대상 정보를 후보 탐색에 이용합니다. 추정된 값은 자격 확정으로 보지 않습니다."],["02","업종·업력","수집 공고에 해당 조건이 명시된 경우 비교합니다. 불명확한 자격은 공고문에서 확인해야 합니다."],["03","지원 분야","금융·기술·인력·수출·내수·창업·경영·기타를 바탕으로 관심 분야를 우선 정렬합니다."]].map(([no,title,detail]) => <article key={no} className="soft-card p-5"><span className="text-xs font-black text-[var(--blue)]">{no}</span><h3 className="mt-3 font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-[#657187]">{detail}</p></article>)}</div></section>
  </>;
}
...
function NoticeCard({ notice, saved, onSave, sessionId, leadId, demo }: { notice: MatchResult; saved: boolean; onSave: () => void; sessionId: string; leadId: string | null; demo: boolean }) {
  ...
  return <article className="soft-card p-6"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-black ${fieldMatched ? "bg-[#e6f8f2] text-[#08795a]" : "bg-[#fff6db] text-[#8d6200]"}`}>{fieldMatched ? "선택 분야 일치" : "추가 확인 후보"}</span><span className="rounded-full bg-[#edf4ff] px-3 py-1 text-xs font-black text-[var(--blue)]">{notice.category}</span>{!demo && notice.dday != null && <span className={`ml-auto text-sm font-black ${notice.dday <= 7 ? "text-[#e24848]" : "text-[#68768d]"}`}>{notice.dday === 0 ? "오늘 마감" : `D-${notice.dday}`}</span>}</div><h3 className="mt-4 text-xl font-black leading-7">{notice.title}</h3><p className="mt-1 text-sm font-bold text-[#778399]">{notice.org}</p>{notice.supportSummary && <p className="mt-4 rounded-xl bg-[#f7faff] px-4 py-3 text-sm font-bold text-[#334764]">{notice.supportSummary}</p>}<ul className="mt-4 flex flex-wrap gap-x-4 gap-y-2">{notice.reasons.map((reason) => <li key={reason} className="text-xs font-bold text-[#4d5f78]">✓ {reason}</li>)}</ul>{notice.checkNote && <p className="mt-4 rounded-lg bg-[#fff8e9] px-3 py-2 text-xs leading-5 text-[#775518]">{notice.checkNote}</p>}<p className="mt-4 text-sm leading-6 text-[#68768d]">{notice.summary}</p>{!demo && <div className="mt-5 flex gap-2"><button onClick={onSave} className={`rounded-lg border px-4 py-3 text-sm font-black ${saved ? "border-[#20b486] bg-[#ecfaf5] text-[#08795a]" : "border-[#cbd8ea] text-[#506078]"}`}>{saved ? "✓ 저장됨" : "☆ 공고 저장"}</button>{notice.url && <a href={notice.url} target="_blank" rel="noreferrer" onClick={() => logEvent("notice_click", sessionId, notice.id, leadId ?? undefined)} className="flex-1 rounded-lg bg-[var(--navy)] px-4 py-3 text-center text-sm font-black text-white">공고문 확인 →</a>}</div>}</article>;
}
